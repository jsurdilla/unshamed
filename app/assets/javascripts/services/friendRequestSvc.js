...
function friendRequestSvc($auth, $rootScope, Friendship, FriendshipRequest, pusherHelperSvc) {
  ...
  self.pendingFriendshipRequests = function() {
    return _.select(self.friendshipRequests, function(fr) {
      return fr.state === 'pending';
    });
  };
  ...
  $auth.validateUser().then(function(user) {
    ...
    // No need to unsubscribe any of the following as this service is created
    // once for the lifetime of the current SPA instance.
    ...
    pusherHelperSvc.subscribeToCancelledFriendReq(function(data) {
      _.remove(self.friendshipRequests, function(friendshipRequest) {
        return friendshipRequest.id === data.friendship_request.id;
      });
      $rootScope.$digest();
    });
  });
