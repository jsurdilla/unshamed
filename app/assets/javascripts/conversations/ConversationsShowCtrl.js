...
ConversationsShowCtrl.$inject = ['$scope', '$rootScope', 'Conversation', 'ConversationList', 'convo', '$auth', '$timeout', 'convoSvc', 'pusherHelperSvc'];
function ConversationsShowCtrl($scope, $rootScope, Conversation, ConversationList, convo, $auth, $timeout, convoSvc, pusherHelperSvc) {
  var vm = this;
  vm.convo = convo;
  ...
  vm.postReply = function() {
    ...
    Conversation.reply({ id: vm.convo.id, body: vm.reply.body }).$promise.then(function(data) {
      ...
      vm.reply.body = '';
      vm.processing = false;
      $rootScope.$broadcast('new-reply-sent', data); // UGLY
    });
  };
