...
TimelineItems.$inject = ['$auth', 'Comment', 'HomeTimelineItem', 'InfiniteScroll', 'Me', 'StatusUpdateForm', 'Timeline'];
function TimelineItems($auth, Comment, HomeTimelineItem, InfiniteScroll, Me, StatusUpdateForm, Timeline) {
  return React.createClass({
    ...
    render: function() {
      var items = this.state.items.map(function(item) {
        return (
          <HomeTimelineItem item={item} key={item.id} />
        );
      });
      ...
      if (this.props.mode === 'user' && $auth.user.id != this.props.userId) {
        ...
        var statusUpdateForm = <StatusUpdateForm onNewStatusUpdate={this.handleNewStatusUpdate} />;
      }
