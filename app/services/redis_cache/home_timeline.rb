module RedisCache
  class HomeTimeline
    ...
    def self.relevant_items_keys(groups)
      ...
      flatten.
      uniq.map { |key| key.gsub(/^timeline:/, '') }.
      select { |key| key.ends_with?(':home_timeline') }
    end
    ...
    def refresh
      ...
      memberPosts = Post.joins(author: :member_profile).where('? = ANY(struggles)', group).order('created_at DESC').first(10_000)
      mhpPosts = Post.joins(author: :mhp_profile).where('? = ANY(struggles)', group).order('created_at DESC').first(10_000)
      journal_entries = JournalEntry.publics.joins(user: :member_profile).where('? = ANY(struggles)', group).order('created_at DESC').first(10_000)
      items = memberPosts + mhpPosts + journal_entries
