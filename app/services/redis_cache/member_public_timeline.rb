module RedisCache
  class MemberPublicTimeline
    ...
    def initialize(user)
      ...
      @multi_group_zset = MultiGroupKeyZset.new('public_timelines', 'items', @groups, self)
    end

    def initial_items(user)
      member_posts = @user.posts.select('posts.id, posts.created_at')
        .order('posts.created_at DESC').first(10_000)
      journal_entries = @user.journal_entries.publics.select('journal_entries.id, journal_entries.created_at')
